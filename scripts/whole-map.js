...
Keep the reference's full canvas framing and ${width}:${height} aspect ratio (ideally ${width} × ${height} pixels). Paint the finished environment directly over the supplied layout without recomposing it. Preserve every room edge, wall centre line, corridor width and door opening on exactly the same reference grid line; never shift architecture by even part of a grid cell. Preserve the reference's exterior margins. Do not add a border, crop the map, move rooms or invent doorways.
Reference legend: yellow lines are native wall boundaries, blue lines are ordinary doors, purple lines are secret doors. Replace these annotations with believable architecture; do not paint the coloured lines, labels, numbers or dashed boxes. Draw each ordinary door closed, straight and centred in its wall opening; never angle an open door leaf across a room or corridor. Secret-door concealment is visual and must be checked by the GM.
