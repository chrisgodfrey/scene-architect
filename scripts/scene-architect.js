...
class SceneArchitectApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static async #importArtwork() {
    ...
    const accepted=await DialogV2.confirm({
      window:{title:`${MODULE_TITLE}: Experimental artwork import`},
      content:"<p>Scene Architect cannot make externally generated artwork match the scene geometry. Import this image only as an unmanaged background?</p>",
      yes:{label:"Continue",icon:"fa-solid fa-triangle-exclamation"},
      no:{label:"Cancel"}
    });
    if(!accepted) return;
    const result=await DialogV2.wait({
      window:{title:"Import unmanaged background"},modal:true,
      content:`<p>Expected canvas: <strong>${expectedW}×${expectedH}px</strong>.</p><input type="file" name="artwork" accept="image/png,image/jpeg,image/webp" style="width:100%"><p class="hint">Foundry may scale the image globally. Local wall, door, floor or grid distortion will remain in the image.</p>`,
      buttons:[{action:"import",label:"Import Background",icon:"fa-solid fa-upload",default:true,callback:(_e,button)=>button.form.elements.artwork.files?.[0]||null},{action:"cancel",label:"Cancel"}],
      rejectClose:false
    });
    ...
    try {
      ...
      if(actualW && (actualW!==expectedW || actualH!==expectedH)) ui.notifications.warn(`${MODULE_TITLE}: artwork is ${actualW}×${actualH}, expected ${expectedW}×${expectedH}. Foundry will fit it to the scene, but compare it carefully with the grid and walls.`);
      ...
      ui.notifications.warn(`${MODULE_TITLE}: unmanaged background imported. Verify it visually against the Foundry grid and walls.`,{permanent:true});
...
Hooks.once("init",()=>{
  console.log(`${MODULE_TITLE} | Initialising v0.1.0-alpha.8`);
